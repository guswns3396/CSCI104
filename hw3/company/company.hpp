...
struct Company {
  ...
  Company ()
  { parent = 0; merge1 = 0; merge2 = 0; }
  Company (Company *m1, Company *m2)
  { parent = 0; merge1 = m1; merge2 = m2; }
};
...
class CompanyTracker {
 ...
 private:
  ...
  // vector to keep track of new companies
  std::vector<Company*> newCompanies;

  Company* findLargest(Company*);
  // finds the largest company the input company belongs to

  bool isValidCompany(int);
  // checks if given input is a valid company (no negatives / out of range)
};
